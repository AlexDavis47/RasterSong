...
impl VideoSignalProcessor {
    ...
    pub fn new(width: u32, height: u32, fps: f32, mapping_mode: MappingMode) -> Self {
        ...
        println!("(Modulator will be stretched {:.2}x to sync with video signal)", speed_mult);
    ...
    pub fn process_frame_at_time(
        ...
        if let Some(ref modulator) = self.modulator_audio {
            ...
            let modulator_slice = self.get_modulator_slice(modulator, &audio_signal, time_seconds, effects);
    ...
    fn get_modulator_slice(&self, modulator: &[f32], audio_signal: &[u8], time_seconds: f32, effects: &EffectSettings) -> Vec<f32> {
        ...
        let filtered_chunk = if effects.lpf_enabled {
            ...
            let u8_chunk: Vec<u8> = original_chunk.iter().map(|&s| ((s * 127.0) + 128.0).clamp(0.0, 255.0) as u8).collect();
            let filtered_u8 = audio_effects::low_pass_filter(&u8_chunk, effects.lpf_cutoff, sample_rate);
            // Convert back to f32
            filtered_u8.iter().map(|&s| ((s as f32 - 128.0) / 127.0).clamp(-1.0, 1.0)).collect()
        } else {
            original_chunk
        };
        
        // Now resample/stretch the filtered chunk to match video signal rate
        // Use linear interpolation for smooth resampling (with full f32 precision)
        let mut slice = Vec::with_capacity(samples_needed);
        for i in 0..samples_needed {
            // Map from video signal sample index to position in filtered chunk (with fractional part)
            ...
            if chunk_idx + 1 < filtered_chunk.len() {
                // Linear interpolation between two samples (full precision)
                ...
                // Last sample (or beyond - use last sample)
                slice.push(filtered_chunk[chunk_idx.min(filtered_chunk.len() - 1)].clamp(-1.0, 1.0));
            } else {
                // Beyond end - pad with silence (neutral = 0.0)
                slice.push(0.0);
            }
        }
    ...
    fn apply_effects(
        ...
        if effects.am_enabled {
            processed = audio_effects::amplitude_modulation(&processed, modulator, effects.am_depth);
        }
        ...
        if effects.fm_enabled {
            processed = audio_effects::frequency_modulation(&processed, modulator, effects.fm_depth);
        }
        
        // Apply signal processing effects with modulation
        if effects.distortion_enabled {
            processed = audio_effects::distortion_modulated(&processed, modulator, effects.distortion_amount);
        }
        
        if effects.bit_crush_enabled {
            processed = audio_effects::bit_crush_modulated(&processed, modulator, effects.bit_crush_bits);
        }
...
mod tests {
    ...
    fn test_process_frame_with_modulator() {
        ...
        assert!(result_brightness > original_brightness, 
                "Expected result to be brighter: {} vs {}", result_brightness, original_brightness);
    }
}
